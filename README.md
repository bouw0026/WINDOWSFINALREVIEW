...
| # | Full Question | MC Style Answer | Concept / Why |
| 9 | What is the most efficient way to enforce a naming convention for Azure resources? | Azure Policy. | **Governance enforcement** – Azure Policy can automatically audit and deny non-compliant names. <br><br>```mermaid\ngraph TD\n    A[Azure Policy] --> B[Check Resource Name]\n    B -->|Compliant| C[Allow]\n    B -->|Non-compliant| D[Deny or Remediate]\n``` |
...
| 15 | Which Azure service provides fine-grained access management for Azure resources? | Azure RBAC. | **Least privilege access** – Assign roles to users, groups, or managed identities. <br><br>```mermaid\ngraph LR\n    A[Azure RBAC] --> B[Owner]\n    A --> C[Contributor]\n    A --> D[Reader]\n``` |
...
| 20 | Which cloud service model requires the most customer-managed security? | IaaS. | **Shared responsibility model** – You manage OS, middleware, and apps. <br><br>```mermaid\ngraph TD\n    A[Customer] -->|IaaS| B[OS, Apps, Data]\n    C[Provider] -->|IaaS| D[Networking, Storage, Virtualization]\n``` |
...
| 39 | When should you scale out? | High load requires more instances. | **Horizontal scaling** – Add more units to handle increased traffic. <br><br>```mermaid\ngraph TD\n    A[Low Load] --> B[1 Instance]\n    C[High Load] --> D[Multiple Instances]\n``` |
